...
TOOL_DEFINITIONS: list[dict] = [
    ...
    {
        "name": "get_party_authority",
        "description": (
            "Look up a party's authority position at a specific date from the "
            "governance authority log. Returns whether the party was actively "
            "appointed, what their authority scope was, and their financial "
            "approval threshold at that date. Use this when assessing whether "
            "a specific action — variation approval, payment certification, "
            "instruction issuance — was within the acting party's authority. "
            "Always call this before concluding that an action was authorised "
            "or unauthorised."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "party_name": {
                    "type": "string",
                    "description": (
                        "The legal name of the party as it appears in the project "
                        "documents. The tool will also attempt to match trading names "
                        "and abbreviations."
                    ),
                },
                "as_of_date": {
                    "type": "string",
                    "description": (
                        "The date at which to assess the party's authority, in "
                        "ISO format YYYY-MM-DD. Pass the date of the action being "
                        "assessed — e.g. the date a variation was approved or an "
                        "instruction was issued. Leave empty to get the latest "
                        "known authority position."
                    ),
                },
            },
            "required": ["party_name"],
...
def _execute_get_party_authority(tool_input: dict, project_id: str) -> dict:
    """
    Deterministic authority position lookup. Zero LLM calls.

    Reads party_identities and authority_events for the specified party
    and date. Returns the party's authority position at that date by
    applying all events up to and including the date in chronological order.

    Used by the Compliance SME to validate whether a specific action
    was within a party's authority at the time it was taken.
    """
    from src.clients import get_supabase_client as _get_supabase

    party_name: str = tool_input.get("party_name", "").strip()
    as_of_date: str = tool_input.get("as_of_date", "").strip()

    if not party_name:
        return {"error": True, "message": "party_name is required."}

    supabase = _get_supabase()

    # Resolve party_name to party_identity_id
    # Try legal_name first, then trading_names
    try:
        result = (
        ...
        identities = result.data or []
    except Exception as exc:
        return {"error": True, "message": f"Failed to query party identities: {exc}"}

    party_identity_id: str | None = None
    party_record: dict | None = None

    for identity in identities:
        if identity["legal_name"].lower() == party_name.lower():
            party_identity_id = identity["id"]
            party_record = identity
            break
        for trading_name in (identity.get("trading_names") or []):
            if trading_name.lower() == party_name.lower():
                party_identity_id = identity["id"]
                party_record = identity
                break
        if party_identity_id:
            break
    ...
    if not party_identity_id:
        return {
            ...
            "message": (
                f"No party named '{party_name}' found in the governance identity "
                f"register for this project. The party may not have been identified "
                f"in the governance run, or the name may differ from the registered "
                f"legal name."
            ),
        }

    # Fetch all authority events for this party, ordered chronologically
    try:
        events_query = (
            supabase.table("authority_events")
            .select(
                "id, event_type, appointment_status, event_date, event_date_certain, "
                "end_date, authority_after, financial_threshold_after, "
                "initiated_by_party_id, authorised_by_party_id, "
                "instrument_status, confirmation_status, missing_action, "
                "assumption_id"
            )
            .eq("project_id", project_id)
        ...
        if as_of_date:
            events_query = events_query.lte("event_date", as_of_date)

        events_result = events_query.execute()
        events = events_result.data or []
    except Exception as exc:
        return {"error": True, "message": f"Failed to query authority events: {exc}"}

    if not events:
        return {
            "error": False,
            "found": True,
            "party_name": party_record["legal_name"],
            "party_category": party_record["party_category"],
            "as_of_date": as_of_date or "all dates",
            "authority_position": "CANNOT CONFIRM — no authority events recorded for this party.",
            "appointment_status": None,
            "assumptions_present": False,
            "events_applied": 0,
            "message": (
                "The party is registered but has no authority events recorded "
                "at or before the specified date. Their authority position cannot "
                "be determined from the governance log."
            ),
        }

    # Apply events chronologically — last non-termination event wins
    # for authority_after and financial_threshold_after
    current_authority: str | None = None
    current_threshold: str | None = None
    current_appointment_status: str = "proposed"
    assumptions_present: bool = False
    active: bool = True

    for event in events:
        if event.get("assumption_id"):
            assumptions_present = True
        if event.get("confirmation_status") == "assumed":
            assumptions_present = True
        ...
        if event_type == "termination":
            active = False
            current_authority = "TERMINATED"
            current_threshold = None
            current_appointment_status = "executed"
        elif event_type == "suspension":
            active = False
            current_authority = f"SUSPENDED — {event.get('authority_after') or 'authority suspended'}"
            current_appointment_status = appt_status
        else:
            # For all other event types, update authority position
            if event.get("authority_after"):
                current_authority = event["authority_after"]
            if event.get("financial_threshold_after"):
                current_threshold = event["financial_threshold_after"]
            current_appointment_status = appt_status
            active = True

    # Build assumption warning
    assumption_warning = ""
    if assumptions_present:
        assumption_warning = (
            " WARNING: One or more events in this party's authority log depend "
            "on user-declared assumptions rather than retrieved documents. "
            "Findings dependent on this authority position should be flagged "
            "as assumption-dependent."
        )
    ...
    return {
        ...
        "party_name": party_record["legal_name"],
        "party_category": party_record["party_category"],
        "entity_type": party_record["entity_type"],
        "as_of_date": as_of_date or "all dates",
        "appointment_status": current_appointment_status,
        "active": active,
        "authority_position": current_authority or "No authority scope recorded.",
        "financial_threshold": current_threshold,
        "assumptions_present": assumptions_present,
        "assumption_warning": assumption_warning,
        "events_applied": len(events),
        "message": (
            f"Authority position for {party_record['legal_name']} "
            f"({'active' if active else 'inactive'}) "
            f"as of {as_of_date or 'latest'}: "
            f"{current_authority or 'no authority recorded'}."
            + assumption_warning
        ),
    }
...
_TOOL_EXECUTORS: dict[str, Callable] = {
    ...
    "get_party_authority": _execute_get_party_authority,
